...
jQuery(document).ready(function ($) {
  ...
  $(window).on('scroll', function () {
    ...
    if($(".mobile-menu").css("display") == "block"){
      var links = $(".mobile-menu").find("img");
    }else{
      var links = $(".main-menu").find("img");
    }
    
    if (scroll < 100) {
      $(".rt-sticky").removeClass("rt-sticky-active ");
      
      for (let index = 0; index < links.length; index++) {
        const element = links[index];
        var currentImg = $(element).attr("src");
        var prevImg = $(element).data("secondary");
        var originalImg = $(element).data("original");
        if(currentImg !== originalImg){
          $(element).attr("src", originalImg);
        }
        //$(element).data("secondary", prevImg);
      }

    } else {
      $(".rt-sticky").addClass("rt-sticky-active ");
      ...
      for (let index = 0; index < links.length; index++) {
        ...
        var originalImg = $(element).data("original");
       // if(currentImg !== originalImg){
        $(element).attr("src", prevImg);
       // }
        
        //$(element).data("secondary", currentImg);
      }
    }
  });
